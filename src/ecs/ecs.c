...
struct cls_ecs_world_query {
    size_t comp_count;
    cls_component *comp_ids;
    struct cls_ecs_world *world;
    struct cls_ecs_world_sparse_set **sets;
...
struct cls_ecs_world_singleton {
    void *data;
    size_t size;
};
...
static void world_destroy(struct cls_ecs_world *world) {
    ...
    while (cls_table_iterator_next(&next, singleton_iter) == CLS_SUCCESS &&
        ...
        if (slot)
            free(((struct cls_ecs_world_singleton *)slot)->data);
    }
...
static cls_error world_init(struct cls_ecs_world *world, bool should_update) {
    ...
    *world = (struct cls_ecs_world){.next_entity_id = 0,
    ...
    error = cls_table_create(&world->components, CLS_ECS_WORLD_START_CAPACITY,
                             sizeof(cls_component),
    ...
    error = cls_table_create(&world->systems, CLS_ECS_WORLD_START_CAPACITY,
                             sizeof(cls_system),
                             sizeof(struct cls_ecs_world_system));
    if (error)
        goto cleanup;

    error = cls_table_create(&world->singletons, CLS_ECS_WORLD_START_CAPACITY,
                             sizeof(cls_singleton),
                             sizeof(struct cls_ecs_world_singleton));
    if (error)
        goto cleanup;
...
cls_error cls_ecs_world_add(struct cls_ecs_world **world, struct cls_ecs *ecs,
    ...
    error = cls_table_insert(ecs->worlds, &id, &w);
    if (error)
        return error;

    void *find_ptr = NULL;
    error = cls_table_find(&find_ptr, ecs->worlds, &id);
    if (error)
        return error;
...
cls_error cls_ecs_world_remove(struct cls_ecs *ecs, cls_world id) {
    ...
    void *world_ptr = NULL;
    cls_error error = cls_table_find(&world_ptr, ecs->worlds, &id);
    if (error)
        return error;

    struct cls_ecs_world *world = world_ptr;
    world_destroy(world);
    return cls_table_remove(NULL, ecs->worlds, &id);
}
...
cls_error cls_ecs_world_component_type_add(struct cls_ecs_world *world,
    ...
    cls_error error = cls_array_create(
        &set.sparse, CLS_ECS_WORLD_ENTITY_START_CAPACITY, sizeof(cls_entity));
    if (error)
        goto cleanup;
    ...
    error = cls_table_insert(world->components, &id, &set);
    if (error)
        goto cleanup;
...
cls_error cls_ecs_world_component_type_remove(struct cls_ecs_world *world,
    ...
    error = cls_table_remove(NULL, world->components, &id);
    if (error)
        return error;
...
cls_error cls_ecs_world_component_add(struct cls_ecs_world *world, cls_entity e,
    ...
    if (e == CLS_ENTITY_MAX || id == CLS_COMPONENT_MAX)
        return CLS_INVALID_ARG;
...
cls_error cls_ecs_world_component_remove(struct cls_ecs_world *world,
    ...
    return world_entity_remove_component_by_id(world, e, id);
}
...
cls_error cls_ecs_world_query_create(struct cls_ecs_world_query **query,
    ...
    struct cls_ecs_world_query *instance =
        calloc(1, sizeof(struct cls_ecs_world_query));
    if (!instance)
    ...
    instance->comp_count = comp_count;

    cls_error error = CLS_SUCCESS;

    instance->comp_ids = malloc(sizeof(*instance->comp_ids) * comp_count);
    if (!instance->comp_ids) {
        error = CLS_OUT_OF_MEMORY;
        goto cleanup;
    }

    instance->sets = (struct cls_ecs_world_sparse_set **)malloc(
        sizeof(*instance->sets) * comp_count);
    if (!instance->sets) {
        error = CLS_OUT_OF_MEMORY;
        goto cleanup;
    }
    ...
    for (size_t i = 0; i < comp_count; ++i) {
        ...
        instance->comp_ids[i] = id;
        instance->sets[i] = set_ptr;
    }

    instance->smallest_set = instance->sets[0];
    if (!instance->smallest_set) {
        error = CLS_INVALID_ARG;
        goto cleanup;
    }
    ...
    for (size_t i = 1; i < comp_count; ++i) {
        struct cls_ecs_world_sparse_set *current_set = instance->sets[i];
...
cls_error cls_ecs_world_query_next(cls_entity *e, void **comps,
    ...
    *e = CLS_ENTITY_MAX;
    ...
    while (query->current_idx < min_dense_len) {
        ...
        for (size_t i = 0; i < query->comp_count; ++i) {
            struct cls_ecs_world_sparse_set *set = query->sets[i];
            if (!set) {
                match = false;
                break;
            }
...
cls_error cls_ecs_world_system_add(struct cls_ecs_world *world, cls_system id,
    ...
    error = cls_table_insert(world->systems, &id, &new_system);
    if (error) {
        if (new_system.query)
...
cls_error cls_ecs_world_system_remove(struct cls_ecs_world *world,
    ...
    void *system_ptr = NULL;
    cls_error error = cls_table_find(&system_ptr, world->systems, &id);
    ...
    cls_ecs_world_query_destroy(system->query);
    return cls_table_remove(NULL, world->systems, &id);
}
...
cls_error cls_ecs_world_singleton_add(struct cls_ecs_world *world,
    ...
    if (cls_table_find(&slot, world->singletons, &id) == CLS_SUCCESS && slot) {
        struct cls_ecs_world_singleton *existing = slot;
    ...
    struct cls_ecs_world_singleton singleton = {.data = copy, .size = size};

    cls_error error = cls_table_insert(world->singletons, &id, &singleton);
...
cls_error cls_ecs_world_singleton_remove(struct cls_ecs_world *world,
    ...
    void *slot = NULL;
    cls_error error = cls_table_find(&slot, world->singletons, &id);
    ...
    return cls_table_remove(NULL, world->singletons, &id);
}
...
cls_error cls_ecs_world_singleton_get(void **out,
    ...
    *out = ((struct cls_ecs_world_singleton *)slot)->data;
    return CLS_SUCCESS;
}
